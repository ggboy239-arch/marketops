...
class Market(commands.Cog):
    ...
    async def market(self, interaction: discord.Interaction):
        ...
        try:
            ...
            await interaction.followup.send(
                "⚠️ MarketOps had trouble building the dashboard. Check the terminal for the error.",
                ephemeral=True,
            )
    ...
    async def market_prefix(self, ctx):
        try:
            ...
            print(f"❌ !market error: {error}")
            await ctx.send("⚠️ MarketOps had trouble building the dashboard. Check the terminal for the error.")
    ...
    def _build_market_embed(self, dashboard):
        ...
        embed.add_field(name="Why?", value=self._format_reasons(dashboard["reasons"]), inline=False)

        embed.add_field(name="🇺🇸 S&P Futures", value=dashboard["assets"]["market"], inline=True)
